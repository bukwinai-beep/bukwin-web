  ...
  return (
    <Section bg="surface" id="demo" className="border-y border-border">
      <Container size="xl">
        ...
        <FadeIn delay={0.2}>
          <div className="mt-12 grid lg:grid-cols-[1.15fr_1fr] gap-6 lg:gap-8 items-stretch">
...
function ChatPanel({ onBooked }: { onBooked: (b: ConfirmedBooking) => void }) {
  ...
  return (
    <div className="rounded-3xl border border-border bg-background overflow-hidden flex flex-col h-[640px]">
        ...
        <AnimatePresence>
          {booking && (
            <motion.div
              ...
              className="mt-4 rounded-2xl bg-secondary/60 p-4"
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">Status: {booking.status}</p>
                </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1 text-muted-foreground">
          <button
            ...
            className="h-7 w-7 inline-flex items-center justify-center rounded-full hover:bg-secondary transition-colors"
          >
            ‹
          </button>
          <button
      ...
      <div className="grid grid-cols-7 gap-1.5 text-center mb-2">
        {dayLabels.map((d, i) => (
          <span key={i} className="text-[10px] font-medium uppercase text-muted-foreground">
            {d}
          </span>
        ))}
      </div>
        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((date) => {
          ...
          return (
            <button
              ...
              disabled={isPast}
              className={cn(
                "aspect-square rounded-full flex items-center justify-center text-xs font-medium transition",
      ...
      {selectedDate && (
        <div className="mt-5 pt-4 border-t border-border">
          <p className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mb-2.5">
